    ...
    public class PayForShoppingCartWorkflow {
        public async Task<IShoppingCartPaidEvent> Execute(PayForShoppingCartCommand command, Func < ClientID, TryAsync<bool> > checkUserExists) {
            ...
            return shoppingCart.Match(
                ...
                whenUnvalidatedShoppingCart: unvalidatedShoppingCart => new ShoppingCartPaymentFailedEvent("Something went down to Georgia and it wasn't the Devil") as IShoppingCartPaidEvent,
                whenValidatedShoppingCart: validatedShoppingCart => new ShoppingCartPaymentFailedEvent("Something went down to Georgia and it wasn't the Devil") as IShoppingCartPaidEvent,
